...
- 実装の多重継承は禁止です。責務の所在が一意でなくなり、合成で常に代替できます。継承する具象基底クラスは1つまでとし、複数の機能が必要ならそれぞれを名詞クラスとして保持し委譲してください。例外は、ライブラリが多重継承を強制する場合（Django のミックスイン等）、およびフレームワーク基底クラス1つ + 実装を持たないマーカー的準拠の併用のみ。実装を持たない型契約（`typing.Protocol` / 本体のない ABC）の複数継承は可です。
